...
class BackupFilesystemTools:
    """Structured filesystem operations. No shell or user-supplied executable is accepted."""
    ...
    def build_plan(
        ...
        if source_info.device_id == destination_info.device_id:
            raise BackupToolError("BACKUP_SAME_FILESYSTEM")
        if "ro" in destination_info.options:
            raise BackupToolError("BACKUP_DESTINATION_READ_ONLY")
        if destination_info.filesystem_type in _INCOMPATIBLE_DESTINATION_FILESYSTEMS:
    ...
    def revalidate_plan(self, plan: BackupPlan) -> None:
        ...
        if source_info.device_id == destination_info.device_id:
            raise BackupToolError("BACKUP_SAME_FILESYSTEM")
        if "ro" in destination_info.options or not os.access(destination, os.W_OK | os.X_OK):
            raise BackupToolError("BACKUP_DESTINATION_NOT_WRITABLE")
        if destination_info.filesystem_type in _INCOMPATIBLE_DESTINATION_FILESYSTEMS:
            raise BackupToolError("BACKUP_DESTINATION_FILESYSTEM_INCOMPATIBLE")
    ...
    async def create_backup(
        ...
        if ares_root.is_symlink() or ares_root.resolve() != ares_root:
        ...
        try:
            ...
            for item, relative in self._walk_copy_entries(
                ...
                if stat.S_ISDIR(info.st_mode):
                    target.mkdir(mode=stat.S_IMODE(info.st_mode), parents=True, exist_ok=False)
                    entry = BackupEntry(
                    ...
                    checksum = await self._copy_regular_file(item, target, info)
                    entry = BackupEntry(
                        relative_path=relative.as_posix(),
            ...
            total_size = sum(item.size_bytes for item in entries if item.entry_type is BackupEntryType.FILE)
            if (
            ...
            self._write_manifest_file(partial, manifest)
    ...
    async def verify_backup(self, backup: Backup, manifest: BackupManifest) -> BackupVerification:
        ...
        for entry in manifest.entries:
            ...
            if source_item.exists() and not source_item.is_symlink():
                try:
                    if await asyncio.to_thread(_hash_file_no_follow, source_item) != checksum:
                        source_changed.append(_path_token(entry.relative_path))
                except OSError:
                    source_changed.append(_path_token(entry.relative_path))

        unexpected = tuple(sorted(self._unexpected_paths(root, expected_paths)))
            ...
            for item, relative, excluded_reason in self._walk_source(root, device_id, policy):
                if excluded_reason is not None:
                    if len(exclusions) < 1024:
                        exclusions.append(
                            BackupExclusion(relative_path=relative.as_posix(), reason=excluded_reason)
                        )
                    continue
                info = os.lstat(item)
                if stat.S_ISDIR(info.st_mode):
    ...
    async def _copy_regular_file(self, source: Path, destination: Path, info: os.stat_result) -> str:
    ...
    def _mount_for(self, path: Path) -> _MountInfo | None:
        ...
        return max(candidates, key=lambda item: len(item.mount_point.parts), default=None)
    ...
    def _canonical_existing(raw: str, code: str) -> Path:
        ...
        _reject_symlink_components(resolved, code)
        return resolved
...
def _plan_fingerprint(plan: BackupPlan) -> str:
    payload = plan.model_dump(mode="json", exclude={"fingerprint_sha256", "created_at", "expires_at"})
    return _sha256_json(payload)


def _manifest_fingerprint(manifest: BackupManifest) -> str:
    payload = manifest.model_dump(
        mode="json", exclude={"manifest_checksum_sha256", "created_at"}
    )
    return _sha256_json(payload)
...
def _path_token(relative_path: str) -> str:
    return hashlib.sha256(relative_path.encode("utf-8", errors="surrogatepass")).hexdigest()[:24]
